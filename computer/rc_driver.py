...
class Move(object):
    ...
    #     # self.rc_car.steer(4) #sound off
    #     time.sleep(5)
    #     self.rc_car.steer(2) #forward
    #     time.sleep(1)
    #     self.rc_car.steer(5) #forward
    #     time.sleep(1)
    #     self.rc_car.steer(9) #stop

class CarMovingHandler(SocketServer.StreamRequestHandler):
    def handle(self):
        global cascade_obj
        global distance_to_car

        if cascade_obj is not None and len(cascade_obj) != 0:
            # first for now
            x_pos, y_pos, width, height = cascade_obj[0]
            ...
            if centred == True:
                ...
                if pr_x != 0:
                    ...
                    self.rc_car.steer(direction)
                    time.sleep(1)
                    self.rc_car.steer(9) #stop

            else:
                if (x_pos > x_cent - width/2) and (x_pos < x_cent + width/2):
                    centred = True

                if x_pos < x_cent - width/2:
                    direction = 1#'right' #1
                elif x_pos > x_cent + width/2:
                    direction = 1#'right' #1
...
class VideoStreamHandler(SocketServer.StreamRequestHandler):
    ...
    # cascade classifiers
    car_cascade = cv2.CascadeClassifier('cascade_xml/car.xml')
    ...
    def handle(self):
        ...
        v_param1, cascade_obj = self.obj_detection.detect(self.car_cascade, gray, image)
        distance_to_car = self.d_to_camera.calculate(v_param1, self.h1, 300, image)
...
class ThreadServer(object):
    ...
    def server_thread2(host, port):
        server = SocketServer.TCPServer((host, port), CarMovingHandler)
        server.serve_forever()
